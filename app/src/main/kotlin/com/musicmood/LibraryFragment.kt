...
class LibraryFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = SongAdapter(
            ...
            onLongClick = { song: Song -> showMoodPicker(song) },
        )
        ...
        view.findViewById<com.google.android.material.button.MaterialButton>(R.id.btnToggleView)
    ...
    private fun showMoodPicker(song: Song) {
        val ctx   = requireContext()
        val sheet = BottomSheetDialog(ctx, R.style.BottomSheetStyle)
        val view  = layoutInflater.inflate(R.layout.sheet_mood_picker, null)
        sheet.setContentView(view)

        view.findViewById<android.widget.TextView>(R.id.tvPickerSong).text =
        ...
        val current = song.effectiveMood
        checks[current]?.let { id ->
            view.findViewById<android.widget.TextView>(id).visibility = android.view.View.VISIBLE
        }

        moods.forEach { (moodName, rowId) ->
            view.findViewById<android.view.View>(rowId).setOnClickListener {
                ...
                val snack = com.google.android.material.snackbar.Snackbar.make(
                    requireView(), "Mood: $moodName",
                    com.google.android.material.snackbar.Snackbar.LENGTH_SHORT)
                snack.setBackgroundTint((0xFF1DB954L).toInt())
                snack.setTextColor((0xFF000000L).toInt())
                snack.show()
            }
        }

        view.findViewById<android.view.View>(R.id.rowResetContainer).setOnClickListener {
            vm.clearMoodOverride(requireContext().applicationContext, song)
            sheet.dismiss()
            com.google.android.material.snackbar.Snackbar.make(
                requireView(), "Mood ripristinato",
