...
class PlayerFragment : Fragment() {
    ...
    private fun updateUI(song: Song) {
        ...
        if (song.analyzed && song.effectiveMood.isNotBlank()) {
            tvMood.text  = song.effectiveMood
            ...
            val color    = SongAdapter.MOOD_COLORS[song.effectiveMood]    ?: (0xFF1DB954L).toInt()
            val colorDim = SongAdapter.MOOD_COLORS_DIM[song.effectiveMood] ?: (0x331DB954L).toInt()
    ...
    internal fun playSong(song: Song) {
        // Se è lo stesso brano già in riproduzione/pausa non riavviare
        if (song.path == currentlyPlayingPath && player != null && !isPreparing) return
        currentlyPlayingPath = song.path
        stopPlayer()   // rilascia il vecchio player con reset() prima di crearne uno nuovo
        isPreparing = true
        btnPlay.text = "▶"
    ...
    private fun stopPlayer() {
        currentlyPlayingPath = null   // forza nuovo avvio al prossimo playSong
        handler.removeCallbacks(updateProgress)
        isPreparing = false
        try { player?.reset() } catch (_: Exception) {}    // reset: funziona da QUALSIASI stato
