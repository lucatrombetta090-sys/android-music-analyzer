...
class SongViewModel : ViewModel() {
    ...
    fun setMoodOverride(context: Context, song: Song, newMood: String) {
        song.moodOverride = newMood
        updateSong(song)
        // Salva immediatamente su cache
        val snapshot = _songs.value ?: return
    ...
    fun getFilteredSongs(): List<Song> {
        ...
        return _songs.value?.filter { s ->
            // Mood: brani non analizzati passano sempre il filtro mood
            val moodOk = f.mood == "Tutti" ||
                (!s.analyzed && f.mood == "Tutti") ||
                s.effectiveMood == f.mood
            // Genre: brani senza genere passano il filtro genre
    ...
    fun loadCache(context: Context) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (cached.isNotEmpty()) {
                ...
                // Applica correzioni mood manuali
                val overrides_done = cached.toMutableList()
                var anyOverride = false
                overrides_done.forEach { s: Song ->
                    val ov = SongCache.getMoodOverride(context, s.path)
                    if (ov != null) { s.mood = ov; anyOverride = true }
                }
                if (anyOverride) _songs.postValue(overrides_done)
    ...
    fun applyMoodOverrides(context: Context) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            var changed = false
            songs.forEach { song ->
                val override = SongCache.getMoodOverride(context, song.path)
                if (override != null && override != song.mood) {
                    song.mood = override
                    changed = true
                }
            }
    ...
    fun startAnalysis(context: Context, folderFilter: String?) {
        ...
        analysisJob = viewModelScope.launch(Dispatchers.IO) {
            try {
                ...
                // Applica correzioni mood manuali
                val overrides_done = finalList.toMutableList()
                var anyOverride = false
                overrides_done.forEach { s: Song ->
                    val ov = SongCache.getMoodOverride(context, s.path)
                    if (ov != null) { s.mood = ov; anyOverride = true }
