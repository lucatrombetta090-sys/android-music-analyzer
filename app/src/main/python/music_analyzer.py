...
def analyze_pcm(pcm_java, sample_rate_int: int) -> str:
    """
    Analisi DSP — algoritmo v4, 9 mood.

    Mood: Energico, Positivo, Aggressivo, Malinconico,
    ...
    try:
        ...
        # ── 4. SMOOTHNESS (morbidezza del suono) ───────────────────────────────
        # Alta smoothness = suono morbido, continuo → romantico, rilassato, nostalgico
        # Bassa smoothness = suono articolato/percussivo → energico, aggressivo
        smoothness = float(np.clip(1.0 - flatness - zcr * 2, 0.0, 1.0))
        ...
        # ── 6. CLASSIFICAZIONE 9 MOOD ──────────────────────────────────────────
        #
        # Aggressivo: distorsione reale (flatness) + energia alta
        is_aggressive = (
        ...
        # Festivo: veloce + positivo + energico ma non aggressivo
        is_festivo = (
            not is_aggressive and
            valence > 0.15 and
            tempo_known and tv > 118 and
            energy_norm > 0.55
        )

        # Energico: veloce + energico, valenza neutra/positiva
        is_energico = (
            not is_aggressive and not is_festivo and
            energy_norm > 0.55 and
            tempo_known and tv > 108
        )

        # Rilassato: lento/medio, bassa energia, morbido
        is_rilassato = (
            not is_aggressive and
            energy_norm < 0.42 and
            smoothness > 0.35 and
            (not tempo_known or tv < 105) and
            valence > -0.10
        )

        # Concentrazione: neutro, stabile, non troppo energico né triste
        is_concentrazione = (
            not is_aggressive and
            abs(valence) < 0.12 and
            energy_norm > 0.25 and energy_norm < 0.60 and
            smoothness > 0.20
        )

        # Romantico: lento/medio, morbido, valenza positiva alta
        is_romantico = (
            not is_aggressive and
            valence > 0.10 and
            smoothness > 0.40 and
            energy_norm < 0.55 and
            (not tempo_known or tv < 110)
        )

        # Nostalgico: tra malinconico e positivo (valenza leggermente negativa + morbido)
        is_nostalgico = (
            not is_aggressive and
            -0.35 < valence < -0.05 and
            smoothness > 0.25 and
            energy_norm < 0.58
        )
